...
def configure(case: Path) -> None:
    ...
    text = re.sub(r'(?m)^(tauInPlane\s+)[^;]+;', r'\g<1>1.0;', text)
    text = re.sub(r'(?m)^(tauThroughPlane\s+)[^;]+;', r'\g<1>2.0;', text)
    ...
    for directory in (case / '0.template',):
        oxygen = directory / 'C_O2'
        text = oxygen.read_text().replace('internalField   uniform 7;', '#include "inletOxygen"\n\ninternalField   uniform $inletOxygen;')
        text = text.replace('value uniform 7;', 'value uniform $inletOxygen;')
        oxygen.write_text(text)
        (directory / 'inletOxygen').write_text('inletOxygen 5.15;\n')
        (directory / 'inletVelocity').write_text('inletVelocity 2.1;\n')
