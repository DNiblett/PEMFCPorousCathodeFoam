...
def scalar_value(path: Path, key: str) -> float:
...
def set_inlet_fields(case: Path, oxygen: float | None = None, velocity: float | None = None) -> None:
    transport = case / 'constant/transportProperties'
    oxygen = scalar_value(transport, 'inletOxygen') if oxygen is None else oxygen
    velocity = scalar_value(transport, 'inletVelocity') if velocity is None else velocity
    oxygen_field = case / '0/C_O2'
    text = oxygen_field.read_text()
    text, internal = re.subn(r'(?m)^(internalField\s+uniform\s+)[^;]+;', rf'\g<1>{oxygen:.8g};', text)
    text, inlet = re.subn(r'(channelInlet[\s\S]*?value\s+uniform\s+)[^;]+;', rf'\g<1>{oxygen:.8g};', text)
...
def patch_flux(case: Path, fields: Path, name: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    with tempfile.TemporaryDirectory(prefix='schneider_vtk_') as temporary:
        temp = Path(temporary)
        for directory in ('constant', 'system', 'dynamicCode'):
...
def run_point(case: Path, flow: Path, output: Path, gas: str, voltage: float,
    ...
    set_inlet_fields(case, oxygen=oxygen)
...
def plot(output: Path, profiles: dict[tuple[str, float], np.ndarray]) -> None:
    ...
    air_voltages = [voltage for voltage in sorted(AIR_DIGITISED)
                    if ('air', voltage) in profiles]
    for axis, voltage in zip(axes.flat, air_voltages):
        ...
        axis.grid(alpha=.25)
    for axis in axes.flat[len(air_voltages):]:
